...
2. 화면이 "물감통 부은" 단색이 아니라 **백룸처럼 지저분하고 으스스하고 불쾌한 사실적 공간**으로 보인다. 누렇게 찌든 벽, 얼룩진 카펫, 깜빡이는 형광등. 방명록이 아무것도 바꾸지 않았을 때의 기본 모습이다.
...
### 5.3 화면 효과
- 필름 그레인·비네트는 넣지 않는다. 분위기는 누런 안개와 형광등 깜빡임으로만 낸다.
...
- **함정**: 앞 칸에 함정이 있고 그 함정을 **알아챘을 때**. 지도에 함정이 표시되면 항상 알아채고, 아니면 처음 마주 볼 때 **5%** 확률로 알아챈다 (눈썰미 좋은 사람만) (함정마다 한 번만 굴린다). 알아채면 서술: "앞 바닥에 이상한 틈이 보인다."
  알아채지 못하면 지금처럼 밟는다.
...
- 지금은 미니맵이 항상 떠서 지나간 칸을 그린다. 이제 `state.map` (방명록에 "지도" 가 반영됨) 일 때만 미니맵 캔버스를 보인다. 없으면 `display: none`.
- 지도가 있을 때의 동작(전체 공개, 함정·괴물 표시)은 기존과 같다.

## 15. 괴물과의 싸움: 회피가 정답

### 15.1 공격은 운이 좋을 때만
- 공격 성공률 (신체 건장한 성인 기준):
  - **권총 80%** (원작: "권총 쓴 새끼 천사인가 덕분에 살았다").
  - 적힌 무기(활·쇠파이프 등, 원거리·근접 모두)·칼 50%.
  - 맨손 30%.
  - 성공: 괴물이 쓰러진다 (한 방에 격파, 기존 hp 3 폐지).
  - 근접 실패: 괴물의 반격. 체력 25 피해, 그중 30% 확률로 무작위 부위를 잃는다.
  - 원거리 실패: 빗나간다. 반격은 없다 (떨어져 있으므로). 괴물은 평소대로 움직인다.
- 몸 상태 보정: `noGrab` 이면 근접 성공률 절반, `blind` 이면 모든 공격 성공률 절반 (겹치면 곱한다).
- 확률은 `public/encounters.js` 의 상수 `COMBAT = { pistol: 0.8, weapon: 0.5, bare: 0.3, counterDamage: 25, counterPartChance: 0.3 }` 로 둔다.

### 15.2 회피는 기본 선택지
- 괴물이 바로 앞에 있으면 기본 선택지 **"몸을 피한다"** 를 항상 보인다. 성공 65%: 옆 빈 칸으로 비켜서고 괴물은 그 턴에 공격하지 못한다. 실패: 체력 15 피해.
  옆이 전부 벽이면 "뒤로 물러선다" 로 바뀐다 (같은 확률).
- 12 절의 특수 행동은 여기에 **추가로** 1개 붙는다. 특수 행동의 성공률은 60~80% 로 유지해서 공격(30%)보다 항상 낫게 한다.
- `slow` (다리·발목을 잃음) 이면 "몸을 피한다" 성공률 35%.

### 15.3 괴물은 한 턴에 두 칸
- 괴물의 턴당 이동 칸 수 = `max(1, round(2 × monsterSpeed))`. 기본 `monsterSpeed` 1 이면 2칸.
- `slow` 이면 플레이어의 이동 한 번이 3턴이라 괴물은 그 사이 6칸 움직인다.
- 괴물이 플레이어 칸에 닿는 순간(이동 도중 포함) 그 턴의 이동을 멈추고 공격한다. 공격 판정은 기존 로직(피해)에 15.1 의 반격 부위 확률을 적용한다.

### 15.4 테스트
- `test/client.js`: 공격 판정 경계 (권총 rng 0.79 성공·0.80 실패, 맨손 0.29·0.30), 몸 상태 보정, 회피 확률, 괴물 이동 칸 수 계산.

## 16. 비용 설정

- `IMAGE_DAILY_LIMIT` 기본값 20 → **5** (`.env.example`, `assets.js` 기본값, README).
