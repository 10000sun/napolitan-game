...
### 15.1 공격은 운이 좋을 때만
- 모든 공격(쏜다·벤다·맨손·적힌 무기)은 **성공 30% / 실패 70%** 로 판정한다 (신체 건장한 성인 기준).
  - 성공: 괴물이 쓰러진다 (한 방에 격파, 기존 hp 3 폐지).
  - 근접 실패: 괴물의 반격. 체력 25 피해, 그중 30% 확률로 무작위 부위를 잃는다.
  - 원거리 실패: 빗나간다. 괴물이 곧바로 한 번 더 움직인다.
- 몸 상태 보정: `noGrab` 이면 근접 성공률 15%, `blind` 이면 모든 공격 성공률 절반.
- 확률은 `public/encounters.js` 의 상수 `COMBAT = { hit: 0.3, counterDamage: 25, counterPartChance: 0.3 }` 로 둔다.
...
### 15.4 테스트
- `test/client.js`: 공격 판정 경계 (rng 0.29 성공, 0.30 실패), 몸 상태 보정, 회피 확률, 괴물 이동 칸 수 계산.
