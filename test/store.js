...
await q.dieRun.run(20, 3, 4, '["오른쪽 팔"]', run.id);
const died = await q.runById.get(run.id);
check(died.death_x === 3, '사망 좌표 칸이 처음부터 있다');
check(died.death_parts === '["오른쪽 팔"]', '죽을 때의 몸도 함께 남는다');
